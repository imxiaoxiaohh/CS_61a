...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        card1 = player.deck.draw()
        card2 = player.deck.draw()
...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if len(player.hand) == 0:
            # print("No cards to add, but player loses the round.")
            return
        else:
            card = player.hand[0]
            copy_card = card.copy()
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand) == 0:
            ...
            card_discard_index = 0
            for index in range(len(player.hand)):
                i_power = player.hand[index].power(opponent_card)
                if i_power > highest_power:
                    highest_power = i_power
                    card_discard_index = index
        best_card = player.hand[card_discard_index]
